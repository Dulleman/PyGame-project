...
bullet_image = pygame.image.load("shot.png")
...
# Fienders funktion
def enemy(x, y, i):
    screen.blit(enemy_image, (x,y, 40, 20))
...
def spawn_boss():
    ...
    if not boss_active:
        ...
        bossY += 2  # Bossen rör sig nedåt

        # Kontrollera om bossen är nära slutet av skärmen
        if bossY >= 600:
            bossX = random.randint(0, 660)  # Ny slumpmässig position inom skärmens bredd
            bossY = 50  # Starta om nära toppen



def boss(x, y):
    screen.blit(boss_image, (x,y,80,40))
...
# Visa poäng
def show_score(x, y):
    score = font.render("Enemies left : " + str(enemies_left), True, WHITE)
